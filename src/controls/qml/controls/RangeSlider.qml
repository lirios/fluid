...
T.RangeSlider {
    ...
    */
    property string firstValueIndicatorText: control.first.value.toLocaleString(
                                                Qt.locale(), "f",
                                                Number.isInteger(control.first.value) ? 0 : 2)
    /*!
        Text displayed in the second handle value indicator.
    */
    property string secondValueIndicatorText: control.second.value.toLocaleString(
                                                 Qt.locale(), "f",
                                                 Number.isInteger(control.second.value) ? 0 : 2)
    ...
    QtObject {
        ...
        readonly property real effectiveHandleWidth: (control.first.pressed || control.second.pressed)
        ...
        readonly property bool reservesLabelSpace:
            control.labelBehavior === RangeSlider.LabelBehavior.WithinBounds
            || control.labelBehavior === RangeSlider.LabelBehavior.Visible
        readonly property real labelSpace: {
            if (!reservesLabelSpace)
                return 0;
            const indicatorSize = control.horizontal
                                  ? Math.max(firstSliderHandle.valueIndicatorHeight,
                                             secondSliderHandle.valueIndicatorHeight)
                                  : Math.max(firstSliderHandle.valueIndicatorWidth,
                                             secondSliderHandle.valueIndicatorWidth);
